...
The native elementary charge used by the solver is
\begin{equation}
e_{\mathrm{native}} = 1.178734 \times 10^{-5}.
\end{equation}
...
For the current experimental implementation, \(d\boldsymbol{\beta}/dt\) and
\(d\gamma/dt\) are derived from the estimated mechanical force:
\begin{align}
...
\end{align}
This keeps the one-dimensional longitudinal terms self-consistent. In the
ideal collinear case, the two Medina bracket terms cancel to numerical
precision. In a transverse bend, \(d\gamma/dt=0\) while
...
Computes a radiation-reaction force vector, applies its impulse to mechanical
momentum, may change both direction and magnitude, and remains experimental
until the mechanical \(\mathbf{F}_{\mathrm{ext}}\) extraction API is validated
for retarded image/source forces.
\end{description}
