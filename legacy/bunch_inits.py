...
def init_bunch(
    ...
    *,
    ...
    q_scalar = (
        charge_sign * 1.178734e-5 * stripped_ions * macro_pop
    )  ##1.6E-19 C => 4.8032047E-10 statC [cm^(3/2)*g^(1/2)*s^(-1)] => [mm^(3/2)*amu^(1/2)*ns^(-1)];
    q = np.full(pcount, q_scalar)

    char_time_scalar = (
        2 / 3 * q_scalar**2 / (mass * c_mmns**3)
    )  # characterstic time for rad. reaction force, see jackson or medina
    char_time = np.full(pcount, char_time_scalar)

    # np.random.seed(4098099)

    Px = (
        np.random.uniform(-transv_mom, transv_mom, pcount) * mass
    )  # 3e-2 amu*mm/ns corresponds to 93 keV
    Py = np.random.uniform(-transv_mom, transv_mom, pcount) * mass
    Pz = (
        np.random.uniform(starting_Pz, starting_Pz + 0.1, pcount) * mass
    )  #  6.3E2 is 2 GeV for protons
