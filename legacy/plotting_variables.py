...
def calculate_plotting_variables(retarded_traj, retarded_drv_traj, init_rider, init_driver, static_steps, ret_steps, E_MeV_rest_rider, E_MeV_rest_driver, c_ms):
    tot_steps = static_steps + ret_steps
    
    delta_e = [np.mean(retarded_traj[i]['gamma'] - init_rider['gamma']) * E_MeV_rest_rider for i in range(2, tot_steps - 1)]
    delta_e_drv = [np.mean(retarded_drv_traj[i]['gamma'] - init_driver['gamma']) * E_MeV_rest_driver for i in range(2, tot_steps - 1)]

    tot_e = [np.mean(retarded_traj[i]['gamma']) * E_MeV_rest_rider for i in range(2, tot_steps - 1)]
    tot_e_drv = [np.mean(retarded_drv_traj[i]['gamma']) * E_MeV_rest_driver for i in range(2, tot_steps - 1)]

    delta_e_keV = np.multiply(delta_e, 1E3)  # convert 1 amu*c^2 to keV
    delta_e_MeV = np.multiply(delta_e, 1)  # convert 1 amu*c^2 to MeV
    tot_e_GeV = np.multiply(tot_e, 1E-3)
    ...
    tot_e_GeV_drv = np.multiply(tot_e_drv, 1e-3)

    zs = [np.mean(retarded_traj[i]['z']) for i in range(2, tot_steps - 1)]
    xs = [np.mean(retarded_traj[i]['x']) for i in range(2, tot_steps - 1)]
    zs_drv = [np.mean(retarded_drv_traj[i]['z']) for i in range(2, tot_steps - 1)]
    xs_drv = [np.mean(retarded_drv_traj[i]['x']) for i in range(2, tot_steps - 1)]
    ...
    return (delta_e, delta_e_drv, tot_e, tot_e_drv, delta_e_keV, delta_e_MeV, tot_e_GeV, delta_e_keV_drv,
            delta_e_MeV_drv, tot_e_GeV_drv, zs, xs, zs_drv, xs_drv, bzs, bxs, bys, bdotxs, bdotys, bdotzs,
