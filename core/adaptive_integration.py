...
class TriggerStatistics:
    ...
    trigger_activations: Optional[Dict[str, int]] = None
    ...
    def activations(self) -> Dict[str, int]:
        """Get trigger activations, ensuring it's never None."""
        if self.trigger_activations is None:
            self.trigger_activations = {trigger.value: 0 for trigger in TriggerType}
        return self.trigger_activations
...
class AdaptiveLienardWiechertIntegrator:
    ...
    def __init__(
        ...
        self.standard_integrator: Union[
            LienardWiechertIntegrator, OptimizedLienardWiechertIntegrator
        ]
        if use_optimized:
        ...
        self.self_consistent_integrator: Optional[
            Union[LienardWiechertIntegrator, OptimizedLienardWiechertIntegrator]
        ] = None

        # Statistics and monitoring
        self.statistics = TriggerStatistics()
        self.previous_force_magnitudes: List[float] = []
        self.previous_positions: List[np.ndarray] = []
    ...
    def _initialize_self_consistent_integrator(self) -> None:
        ...
        if self.self_consistent_integrator is None:
            # Use the optimized integrator for self-consistent mode as well
            # since it contains the full physics implementation
            if self.use_optimized:
                self.self_consistent_integrator = OptimizedLienardWiechertIntegrator()
            else:
                self.self_consistent_integrator = LienardWiechertIntegrator()

    def _calculate_force_magnitude(self, dPx: float, dPy: float, dPz: float) -> float:
        """Calculate magnitude of 3-force vector."""
        return float(np.sqrt(dPx**2 + dPy**2 + dPz**2))

    def _calculate_acceleration_magnitude(
        self, force_magnitude: float, gamma: float, mass: float
    ) -> float:
        """Calculate magnitude of 3-acceleration."""
        return force_magnitude / (gamma * mass * C_MMNS**2)

    def _calculate_relative_energy_change(
        self, dPt: float, total_energy: float
    ) -> float:
        """Calculate relative energy change."""
        return abs(dPt) / total_energy if total_energy > 0 else 0.0

    def _calculate_field_gradient(
        self, current_force_mag: float, current_position: np.ndarray
    ) -> float:
        ...
        position_change = float(np.linalg.norm(current_position - prev_position))
    ...
    def _check_triggers(
        ...
        relative_energy_change = self._calculate_relative_energy_change(
            dPt, total_energy
        )
        ...
        if normalized_force > self.thresholds.force_threshold:
            ...
            self.statistics.activations[TriggerType.FORCE_MAGNITUDE.value] += 1
        ...
        if acceleration_magnitude > self.thresholds.acceleration_threshold:
            ...
            self.statistics.activations[TriggerType.ACCELERATION_MAGNITUDE.value] += 1
        ...
        if relative_energy_change > self.thresholds.energy_change_threshold:
            ...
            self.statistics.activations[TriggerType.RELATIVE_ENERGY_CHANGE.value] += 1

        if field_gradient > self.thresholds.field_gradient_threshold:
            trigger_reasons.append(f"Field gradient: {field_gradient:.2e}")
            self.statistics.activations[TriggerType.FIELD_GRADIENT.value] += 1
        ...
        if should_trigger and self.debug_mode:
            ...
            print(
                f"   Force: {normalized_force:.2e}, Accel: {acceleration_magnitude:.2e}"
            )
            print(
                f"   Energy: {relative_energy_change:.2e}, Gradient: {field_gradient:.2e}"
            )

        return should_trigger, trigger_reason

    def adaptive_integration_step(
        self,
        h_step: float,
    ...
    ) -> Any:  # Using Any to avoid mypy inference issues with dynamic method dispatch
        """
        ...
        if i_traj > 0:
            ...
            dPt = (
                current.get("Pt", gamma * mass * C_MMNS**2)
                - previous.get("Pt", gamma * mass * C_MMNS**2)
            ) / h_step
            ...
            if should_trigger:
                ...
                # Mypy needs to know this is not None after initialization
                assert self.self_consistent_integrator is not None
                result = self.self_consistent_integrator.self_consistent_enhanced_step(
                    h_step, trajectory, trajectory_drv, i_traj, apt_R, sim_type
                )
                return result
    ...
    def integrate(
        ...
        wall_Z: float,
        apt_R: float,
        sim_type: SimulationType = SimulationType.FREE_PARTICLE_BUNCHES,
        ...
        if self.debug_mode:
            ...
            print(
                f"   Thresholds - Force: {self.thresholds.force_threshold:.1e}, "
        ...
        # Final statistics
        self_consistent_percentage = (
            100.0 * self.statistics.self_consistent_steps / self.statistics.total_steps
        )

        if self.debug_mode:
            print("✅ Adaptive integration complete!")
            print(
                f"   Self-consistent steps: {self.statistics.self_consistent_steps}/{self.statistics.total_steps} "
                f"({self_consistent_percentage:.1f}%)"
            )
            print(f"   Max force: {self.statistics.max_force_magnitude:.2e}")
            print(
                f"   Max acceleration: {self.statistics.max_acceleration_magnitude:.2e}"
            )
            print(f"   Max energy change: {self.statistics.max_energy_change:.2e}")

            for trigger_type, count in self.statistics.activations.items():
                if count > 0:
