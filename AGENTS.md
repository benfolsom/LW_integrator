...
Default integration-style test-run parameters (unless the user explicitly overrides them): use prehistory separation `1000 mm`, total steps around `1200`, auto timestep calculation from relative closing speed (rather than fixed manual timestep), and trajectory sampling/output every `100` steps. When bypassing the CLI/GUI and calling the proper-time integrator directly, convert a desired lab-frame propagation distance to proper-time step size using `h_step = distance_mm / (gamma * beta * C_MMNS * (steps - 1))` for single-bunch coasting beams; do not use the lab-time `distance / (beta c steps)` value directly.
