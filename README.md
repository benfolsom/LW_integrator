...
- **General 3D particle initialization** — `create_particle_state_3d` supports arbitrary bunch orientation (`momentum_axis`), starting position, and longitudinal span. When `momentum_axis` is present in rider/driver config params, the 3D initializer is used automatically in both the testbed and CLI single-run paths. Backward-compatible with existing z-axis configs.
