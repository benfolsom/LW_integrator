...
class StandardParticleProperties:
    ...
    @classmethod
    def get_standard_properties(
        cls, particle_type: ParticleType
    ) -> Optional["StandardParticleProperties"]:
...
class BeamParameters:
    ...
    @classmethod
    def from_energy(
        cls, particle_type: ParticleType, energy_mev: float, **kwargs: Any
    ) -> "BeamParameters":
...
class SimulationParameters:
    ...
    integration_method: str = (
        "adaptive"  # 'basic', 'optimized', 'adaptive', 'self_consistent'
    )
...
class StandardInputFormat:
        ...
        if "beam" in data:
            ...
            if "total_energy_mev" in beam_data:
                ...
                self.beam_parameters = BeamParameters(
                    particle_type=particle_type,
                    **{k: v for k, v in beam_data.items() if k != "particle_type"},
                )
        ...
        if "lattice" in data:
            ...
            for elem_data in data["lattice"]:
                ...
                element_type = elem_data.get(
                    "type", elem_data.get("element_type", "drift")
                )
    ...
    def _load_elegant(self, filename: Path) -> None:
        ...
        with open(filename, "r") as f:
            f.readlines()  # Read file but don't store - simplified parser
    ...
    def save_to_file(
        self, filename: Union[str, Path], format_type: str = "auto"
    ) -> None:
        """
...
def create_example_configurations() -> (
    Tuple["StandardInputFormat", "StandardInputFormat"]
):
...
def main() -> None:
    ...
    doc_example = {
        ...
        "lattice": [
            ...
            },
            {
                ...
                "length": 0.3,
                "strength": 2.5,
                "aperture": {"aperture_type": "circular", "aperture_limits": [0.005]},
            },
        ],
        "simulation": {
            "n_turns": 0,
    ...
    if loader.beam_parameters:
        print(
            f"Loaded beam: {loader.beam_parameters.particle_type.value} at {loader.beam_parameters.total_energy_mev} MeV"
        )
