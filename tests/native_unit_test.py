        ...
        try:
            ...
            if isinstance(result, tuple) and len(result) == 2:
                ...
                if 100 <= kinetic_energy_mev <= 10000:  # 0.1-10 GeV
                    print(f"  → Reasonable energy range for accelerator physics")
                elif kinetic_energy_mev < 100:
                    print(f"  → Low energy (sub-relativistic)")
                else:
                    print(f"  → Very high energy")
...
def create_unit_converter():
    ...
    if si_error < 0.1 and legacy_error < 0.1:
        ...
        print(f"  ✗ Unit conversion has errors")
        return None, None
            ...
            for apt_r in aperture_sizes:
                ...
                try:
                    ...
                    if isinstance(field_result, dict):
                        ...
                        if max_force > 1e3:
                            print(f"  → Significant electromagnetic forces")
                        else:
                            print(f"  → Weak electromagnetic forces")
...
def test_particle_propagation_native():
    ...
    if isinstance(result, tuple):
        ...
        if abs(distance_traveled - expected_distance) < 1e-10:
            print(f"  ✓ Perfect free particle motion")
        else:
            print(f"  ⚠ Numerical integration error")
