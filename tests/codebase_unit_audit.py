...
def audit_file_units(filepath):
    ...
    if import_lines:
        ...
        for line in import_lines[:5]:
            if any(
                word in line.lower() for word in ["physics", "constants", "particle"]
            ):
                print(f"  {line}")
...
def analyze_unit_conversions():
    ...
    try:
        ...
        print("Test particle (1 GeV proton):")
        print(f"  γ = {gamma:.6f}")
        print(f"  Pz = {pz:.6e}")
        print(f"  Pt = {pt:.6e}")
        ...
        if abs(ratio_amu - 1) < abs(ratio_si - 1):
            print("  → Core system using amu*mm/ns units")
            core_units = "amu*mm/ns"
        else:
            print("  → Core system using SI units")
            core_units = "SI"
...
def create_unit_consistency_test():
    ...
    if core_units == "amu*mm/ns":
        ...
        print("Internal consistency check (amu*mm/ns):")
        ...
        print("Internal consistency check (SI):")
        print(f"  Expected Pt = γmc = {expected_pt:.3e}")
        print(f"  Actual Pt = {pt:.3e}")
        print(f"  Expected |p| = {expected_p:.3e}")
        print(f"  Actual |p| = {p_magnitude:.3e}")
    ...
    if abs(pt_ratio - 1) < 0.01 and abs(p_ratio - 1) < 0.01:
        ...
        print("  ✗ INCONSISTENT - unit mixing detected")
        consistent = False
...
def main():
    ...
    if not consistent:
        print("\nFIXES NEEDED:")
        print("1. Convert all mass constants from kg to amu")
        print("2. Convert all velocity constants from m/s to mm/ns")
        print("3. Ensure momentum calculations use amu*mm/ns")
        print("4. Verify electromagnetic charge in Gaussian units")
    else:
        print("\n✓ Unit system appears consistent!")

    print("\nNEXT STEPS:")
    print("1. Build comprehensive physics test with corrected units")
    print("2. Compare legacy and core system outputs directly")
    print("3. Verify electromagnetic field calculations")
    print("4. Test aperture physics with consistent momentum")
