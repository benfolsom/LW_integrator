...
class EnhancedApertureTest:
    ...
    def run_enhanced_test(self):
        ...
        print(f"Wall distance: {(self.aperture_radius - self.particle_y)*1000:.1f}μm (very close!)")
        ...
        print(f"Initial energy: {initial_energy:.1f} MeV ({initial_energy/1000:.2f} GeV)")
        ...
        while z < self.end_z and step < 5000:
            ...
            if abs(z - self.aperture_z) < 25.0:  # Within ±25mm of aperture
                force_x, force_y, force_z = self.apply_enhanced_em_force(x_pos, y_pos, z)
                ...
                if step % 200 == 0 and force_magnitude > 1e-10:
                    print(f"  Step {step}: z={z:.1f}mm, F={force_magnitude:.2e}, E={energy_mev:.1f}MeV")
    ...
    def create_enhanced_plot(self, z_positions, energies_mev, forces, initial_energy):
        ...
        ax1.axhline(y=initial_energy, color='r', linestyle='--', alpha=0.7, label='Initial energy')
        ...
        plt.tight_layout()
        plt.savefig('/home/benfol/work/LW_windows/tests/enhanced_aperture_test.png', dpi=150, bbox_inches='tight')
