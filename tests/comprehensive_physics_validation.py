...
class ComprehensivePhysicsTest:
        ...
        try:
            ...
            for pz in test_momenta:
                ...
                try:
                    ...
                    acceleration_y = em_result['bdoty'][0] if 'bdoty' in em_result else 0.0
                    ...
                    acceleration_y2 = em_result2['bdoty'][0] if 'bdoty' in em_result2 else 0.0
                    ...
                    if abs(acceleration_y) > 1e-10 and abs(acceleration_y2) > 1e-10:
                        ...
                        scaling_match = abs(scaling_ratio - accel_ratio) / scaling_ratio < 0.5
                        
                        print(f"  1/r² scaling: {scaling_match} (ratio: {accel_ratio:.2f}, expected: {scaling_ratio:.2f})")
                    else:
                        print(f"  1/r² scaling: No significant force detected")
                    
                    if abs(acceleration_y) > 1e-6:  # Significant acceleration
                        print(f"  ✓ Electromagnetic field detected")
                    else:
                        print(f"  ⚠ No significant electromagnetic field")
    ...
    def test_legacy_energy_conservation(self):
        ...
        try:
            ...
            if relative_error < 1e-15:  # Machine precision
                ...
                print(f"  ⚠ Energy not perfectly conserved")
    ...
    def test_core_system_physics(self):
        ...
        try:
            ...
            for energy_mev in test_energies:
                ...
                gamma = bunch['gamma'][0]
                momentum_kg_ms = np.sqrt(bunch['Px'][0]**2 + bunch['Py'][0]**2 + bunch['Pz'][0]**2)
                beta = np.sqrt(1 - 1/gamma**2)
                
                # Verify relativistic relationships
                rest_energy_mev = proton_species.mass_kg * (2.998e8)**2 / (1.602e-19 * 1e6)
                ...
                print(f"  Energy check: {calculated_energy:.1f} MeV (error: {energy_error:.2e})")
                ...
                momentum_from_energy = np.sqrt(total_energy_j**2 - rest_mass_energy**2) / 2.998e8
                momentum_error = abs(momentum_from_energy - momentum_kg_ms) / momentum_kg_ms
                
                print(f"  Momentum verification: {momentum_from_energy:.6e} kg*m/s (error: {momentum_error:.2e})")
                
                if energy_error < 1e-10 and momentum_error < 1e-10:
                    print(f"  ✓ Perfect relativistic consistency")
                else:
                    print(f"  ⚠ Small numerical errors")
    ...
    def test_aperture_physics_scenario(self):
        ...
        try:
            ...
            for y_pos in test_positions:
                if y_pos < aperture_radius:  # Inside aperture
                    ...
                    try:
                        ...
                        print(f"  y = {y_pos*1000:2.0f} μm: distance = {wall_distance*1000:2.0f} μm, acceleration = {acceleration_y:.2e}")
            ...
            print(f"  Track proton from z = -200 mm to z = +200 mm")
            print(f"  Aperture at z = 0 mm")
            print(f"  Expect: Energy increase due to EM acceleration near aperture")
            print(f"  Current energy: {energy_mev:.1f} MeV")
            print(f"  With EM forces ~10⁵, expect significant acceleration")
    ...
    def run_all_tests(self):
        ...
        if passed == total:
            print(f"\n🎯 ALL TESTS PASSED!")
            print(f"Both physics systems are working correctly in their native units.")
            print(f"Ready for aperture physics simulations!")
        else:
