...
class TestPhysicsAccuracy:
    ...
    def test_particle_state(self):
        ...
        return {
            ...
            'Px': np.array([0.1]), 'Py': np.array([0.05]), 'Pz': np.array([0.2]),
            'q': np.array([1.0]), 'mass': np.array([1.0]), 'gamma': np.array([1.05])
        }
    
    @pytest.mark.physics
    def test_basic_vs_optimized_agreement(self, basic_integrator, optimized_integrator, 
                                        test_particle_state):
    ...
    def test_energy_momentum_consistency(self, test_particle_state, energy_mev):
        ...
        assert relative_error < 1e-12, f"Energy-momentum inconsistency: {relative_error:.2e}"
    ...
    def test_unit_conversion_consistency(self):
        ...
        print(f"✅ Unit conversion factor: {conversion_factor:.1f} (expected {expected_factor})")
    ...
    def test_gamma_beta_consistency(self, gamma):
        ...
        print(f"✅ γ={gamma:.2f}, β={beta:.4f}: consistency error = {relative_error:.2e}")
...
class TestRelativisticInvariants:
    ...
    def test_lorentz_invariant_interval(self, boost_velocity):
        ...
        assert relative_error < 1e-12, f"Lorentz invariant violation: {relative_error:.2e}"
        
        print(f"✅ Boost v={boost_velocity:.1f}c: interval invariance error = {relative_error:.2e}")
...
class TestNumericalStability:
    ...
    def test_numerical_precision_thresholds(self, precision):
        ...
        if precision >= 1e-15:  # Within double precision
            ...
            assert relative_error < 10 * precision, f"Precision loss: {relative_error:.2e}"
            print(f"✅ Precision {precision:.0e}: relative error = {relative_error:.2e}")
        else:
