...
try:
    from lw_integrator.core.adaptive_integration import AdaptiveIntegrator, TriggerThresholds  
...
class MacroParticleEnsemble:
    ...
    def current_population(self) -> int:
        return sum(mp.effective_population for mp in self.macroparticles if not mp.is_lost)
    ...
    def check_wall_collisions(self, aperture_radius: float, current_time: float):
        ...
        for mp in self.macroparticles:
            ...
            if r >= aperture_radius * 0.9 and not mp.is_lost:
                bleeding_fraction = min(0.5, (r - aperture_radius * 0.9) / (aperture_radius * 0.1))
    ...
    def _execute_population_bleeding(self, source_mp: MacroParticle, current_time: float):
        """
        ...
        for mp in self.active_macroparticles:
            ...
            if distance <= bleeding_radius and mp.radial_distance < source_mp.radial_distance:
    ...
    def get_bleeding_statistics(self) -> Dict:
        ...
        total_transferred = sum(event['population_transferred'] for event in self.bleeding_log)
        
        return {
            'total_events': len(self.bleeding_log),
            'total_population_transferred': total_transferred,
            'average_transfer_distance': np.mean([event['distance'] for event in self.bleeding_log]),
...
def run_aperture_test(
    ...
    print(f"Running test: aperture={aperture_radius:.3f}mm, particles={particle_count}, beam={beam_size:.3f}mm")
    ...
    # Setup integrator
    integrator = setup_adaptive_integrator(config)
    ...
    for step, t in enumerate(times):
        if step % 100 == 0:
            print(f"  Step {step}/{num_steps}, survival: {ensemble.survival_fraction:.3f}")
        ...
        avg_mom = np.mean([mp.momentum for mp in active_mp], axis=0)
    ...
    results = {
        ...
        'final_stats': {
            ...
            'self_consistent_fraction': np.mean(self_consistency_triggers) if self_consistency_triggers else 0,
            ...
            'population_conservation': {
                ...
                'conservation_ratio': ensemble.total_population_including_bleeding / initial_population
            }
        },
    ...
    for aperture in config.aperture_sizes:
        for particle_count in config.particle_counts:
            for beam_size in config.beam_sizes:
                ...
                try:
                    result = run_aperture_test(config, aperture, particle_count, beam_size)
                    all_results.append(result)
                except Exception as e:
                    print(f"Error in test aperture={aperture}, particles={particle_count}: {e}")
                    continue
...
def analyze_results(results: List[Dict]) -> Dict:
    ...
    self_consistent_fractions = [r['final_stats']['self_consistent_fraction'] for r in results]
    ...
    summary = {
        ...
        'self_consistency_usage': {
            ...
            'tests_with_triggers': sum(1 for f in self_consistent_fractions if f > 0.01)
        }
    }
    
    # Identify problematic configurations
    high_loss_tests = [r for r in results if r['final_stats']['final_survival_fraction'] < 0.5]
...
def create_verification_plots(results: Dict, output_dir: Path):
    ...
    survivals = [r['final_stats']['final_survival_fraction'] for r in results['individual_results']]
    particles = [r['config']['particle_count'] for r in results['individual_results']]
    
    # Color by particle count
    scatter = plt.scatter(apertures, survivals, c=particles, cmap='viridis', alpha=0.7, s=50)
    ...
    sc_fractions = [r['final_stats']['self_consistent_fraction'] for r in results['individual_results']]
