...
class TestRadiationReaction:
    ...
    @pytest.fixture
    def realistic_electron_params(self):
    ...
    @pytest.mark.unit
    def test_realistic_char_time_calculation(self, realistic_electron_params):
    ...
    @pytest.mark.unit
    @pytest.mark.parametrize("gamma,expected_scaling", [
        (100, "low"),      # 50 MeV electron
        (1956, "moderate"), # 1 GeV electron  
        (10000, "high"),   # 5 GeV electron
    ])
    def test_radiation_reaction_scaling(self, integrator, realistic_electron_params, 
        ...
        radiation_force_factor = (2/3 * params['charge']**2 / (params['mass'] * C_MMNS**3) 
                                * acceleration_magnitude)
        
        relative_change = radiation_force_factor * timestep / (gamma * params['mass'] * C_MMNS**2)
        
        # Scaling expectations based on energy
        if expected_scaling == "low":
            assert relative_change < 1e-6, f"Low energy: change too large {relative_change:.2e}"
        elif expected_scaling == "moderate": 
            assert relative_change < 1e-3, f"Moderate energy: change too large {relative_change:.2e}"
        elif expected_scaling == "high":
            assert relative_change < 1e-1, f"High energy: change too large {relative_change:.2e}"
        
        print(f"✅ {expected_scaling} energy (γ={gamma}): relative change = {relative_change:.2e}")
    
    @pytest.mark.unit
    def test_radiation_reaction_direction(self, integrator, realistic_electron_params):
    ...
    @pytest.mark.unit
    def test_energy_conservation_violation(self, realistic_electron_params):
        ...
        initial_energy = params['gamma'] * params['mass'] * C_MMNS**2
        
        # After radiation reaction, energy should decrease
        # This tests the physics principle
        
        energy_loss_rate = (2/3 * params['charge']**2 / (4 * np.pi * 8.854e-12) 
                           * (1e15)**2 / C_MMNS**3)  # Larmor formula
        
        assert energy_loss_rate > 0, "Energy loss rate must be positive"
        
        print(f"✅ Energy loss rate: {energy_loss_rate:.2e} (violates conservation as expected)")
    
    @pytest.mark.unit 
    @pytest.mark.parametrize("timestep", [1e-9, 1e-6, 1e-3])
    def test_timestep_independence(self, realistic_electron_params, timestep):
        ...
        if timestep < char_time:
            assert rate_factor > 1, f"Rate factor should be > 1 for dt < char_time"
...
class TestRadiationReactionIntegration:
    ...
    @pytest.mark.slow
    def test_radiation_reaction_energy_loss_trajectory(self):
    ...
    @pytest.mark.integration  
    def test_radiation_reaction_vs_classical(self):
...
class TestRadiationReactionPerformance:
    ...
    @pytest.mark.slow
    def test_radiation_reaction_performance(self, benchmark):
...
def generate_test_particle_data(n_particles=100, energy_range=(1, 1000)):
    """Generate test particle data for radiation reaction tests."""
    energies = np.logspace(np.log10(energy_range[0]), np.log10(energy_range[1]), n_particles)
    gammas = energies / (ELECTRON_MASS * C_MMNS**2)  # Convert energy to gamma
    
    return {
        'gammas': gammas,
        'masses': np.full(n_particles, ELECTRON_MASS),
