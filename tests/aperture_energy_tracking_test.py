...
class ApertureEnergyTracker:
        ...
        try:
            ...
            if abs(z_position - self.aperture_z) < 50.0:
                ...
                if r_from_center < self.aperture_radius:
                    ...
                    force_scale = 1.0  # Base Coulomb constant in Gaussian amu*mm*ns units
                    force_magnitude = force_scale * q_particle * abs(q_image) / (wall_distance**2)
    ...
    def propagate_with_em_fields(self):
        ...
        while z < self.end_z and step < 10000:  # Safety limit
            ...
            accel_x, accel_y, accel_z = self.apply_electromagnetic_force(particle_vector, z)
            ...
            if step % 1000 == 0 and abs(accel_y) > 1e-10:
                print(f"    z={z:.1f}mm: accel_y={accel_y:.2e}, dpz={dpz_dt*self.dt:.2e}")
            ...
            # Progress update
            if step % 500 == 0:
                print(f"  Step {step}: z={z:.1f}mm, E={energy:.1f}amu*mm²/ns², γ={current_gamma:.3f}")
            
            step += 1
            
        return np.array(z_positions), np.array(energies), np.array(gammas), np.array(em_forces)
    ...
    def plot_results(self, z_em, energy_em, gamma_em, em_forces, z_free, energy_free):
        ...
        ax1.axvline(x=self.aperture_z, color='k', linestyle=':', alpha=0.7, label='Aperture')
        ...
        ax2.axvline(x=self.aperture_z, color='k', linestyle=':', alpha=0.7, label='Aperture')
        ...
        ax3.axvline(x=self.aperture_z, color='k', linestyle=':', alpha=0.7, label='Aperture')
        ...
        ax4.axvline(x=self.aperture_z, color='k', linestyle=':', alpha=0.7, label='Aperture')
        ...
        plt.tight_layout()
        plt.savefig('/home/benfol/work/LW_windows/tests/aperture_energy_tracking.png', dpi=150, bbox_inches='tight')
    ...
    def run_test(self):
        ...
        print(f"Aperture: radius={self.aperture_radius*1000:.1f}μm at z={self.aperture_z}mm")
        ...
        initial_energy_gev = initial_energy * self.mp_amu * 931.494 / (self.c_mmns**2) / 1000
        ...
        print(f"Free space energy change: {np.max(energy_free_gev) - energy_free_gev[0]*1000:.3f} MeV (should be ~0)")
