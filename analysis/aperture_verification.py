...
class TrajectoryData:
    ...
    radiation_power: List[float] = field(
    ...
    def add_point(self, position, velocity, energy, time_val, wall_distance, step_num):
        ...
        acceleration = np.linalg.norm(
            velocity - (self.velocities[-2] if len(self.velocities) > 1 else velocity)
        )
        power = (2 * ELEMENTARY_CHARGE**2 * acceleration**2) / (
            3 * 4 * np.pi * 8.854187817e-12 * SPEED_OF_LIGHT**3
        )
    ...
    def get_summary(self):
        ...
        return {
            ...
            "energy_initial_mev": (
                self.kinetic_energies_mev[0] if self.kinetic_energies_mev else 0
            ),
            "energy_final_mev": (
                self.kinetic_energies_mev[-1] if self.kinetic_energies_mev else 0
            ),
            "energy_loss_mev": (
                self.kinetic_energies_mev[0] - self.kinetic_energies_mev[-1]
                if len(self.kinetic_energies_mev) > 1
                else 0
            ),
            "min_wall_distance": (
                min(self.wall_distances) if self.wall_distances else float("inf")
            ),
            "max_radiation_power": (
                max(self.radiation_power) if self.radiation_power else 0
            ),
...
class EnhancedMacroParticle:
    ...
    def __init__(
        self, position, velocity, charge, mass, particle_id, initial_energy_mev=10000
    ):
        ...
        # Set initial energy
        self.total_energy = (
            initial_energy_mev * ELEMENTARY_CHARGE * 1e6
        ...
        self.close_approaches = (
            []
        )  # List of (time, distance, position) for close wall approaches
    ...
    def check_wall_collision(self, aperture_radius, close_threshold=10e-6):
        ...
        if wall_distance < close_threshold and wall_distance > 0:
            self.close_approaches.append(
                (self.time, wall_distance, self.position.copy())
            )
...
def enhanced_beam_initialization(
    n_particles, beam_sigma, aperture_radius, energy_mev=10000
):
    ...
    if beam_sigma > aperture_radius * 0.4:
        print(
            f"Warning: Large beam sigma ({beam_sigma*1000:.2f}mm) vs aperture ({aperture_radius*1000:.1f}mm)"
        )
        print(
            f"Recommended sigma: <{recommended_sigma*1000:.2f}mm for better survival rates"
        )
    ...
    initial_radii = [
        np.sqrt(p.position[0] ** 2 + p.position[1] ** 2) for p in particles
    ]
    inside_aperture = sum(1 for r in initial_radii if r < aperture_radius)

    print(
        f"  Initial inside aperture: {inside_aperture}/{n_particles} ({inside_aperture/n_particles:.1%})"
    )
...
def run_enhanced_simulation(
    particles, aperture_radius, total_length=2.0, n_steps=100, save_frequency=3
):
    ...
    total_time = (
        total_length / SPEED_OF_LIGHT
    )  # Time to traverse length at speed of light
    ...
    for step in range(n_steps):
        if step % max(1, n_steps // 10) == 0:
            avg_energy = (
                np.mean([p.get_kinetic_energy_mev() for p in alive_particles])
                if alive_particles
                else 0
            )
            print(
                f"  Step {step:3d}/{n_steps}: {len(alive_particles):3d} alive, avg E: {avg_energy:.1f} MeV"
            )
    ...
    print(
        f"  Survival: {results['survival_rate']:.1%} ({len(alive_particles)}/{len(particles)})"
    )
...
def run_optimized_test_suite():
    ...
    test_configs = [
        {
            ...
            "n_particles": 25,
            "steps": 50,
        },  # 5mm aperture, 1mm beam
        {
            "aperture": 2e-3,
            "beam_sigma": 0.4e-3,
            "n_particles": 30,
            "steps": 75,
        },  # 2mm aperture, 0.4mm beam
        {
            "aperture": 1e-3,
            "beam_sigma": 0.2e-3,
            "n_particles": 35,
            "steps": 100,
        },  # 1mm aperture, 0.2mm beam
        {
            "aperture": 0.5e-3,
            "beam_sigma": 0.1e-3,
            "n_particles": 30,
            "steps": 125,
        },  # 0.5mm aperture, 0.1mm beam
        {
            "aperture": 0.3e-3,
            "beam_sigma": 0.06e-3,
            "n_particles": 25,
            "steps": 150,
        },  # 0.3mm aperture, 0.06mm beam
        {
            "aperture": 0.1e-3,
            "beam_sigma": 0.02e-3,
            "n_particles": 20,
            "steps": 200,
        },  # 0.1mm aperture, 0.02mm beam
    ]
    ...
    for i, config in enumerate(test_configs):
        ...
        if results["energy_evolution"]:
            ...
            print(
                f"  Energy Loss: {energy_loss:.3f} MeV ({energy_loss/initial_energy:.2%})"
            )
